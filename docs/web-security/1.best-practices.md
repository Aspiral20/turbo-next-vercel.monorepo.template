...
- Browser: store in memory only (JavaScript variable, React state).
  - ❌ Avoid localStorage/sessionStorage because of XSS risks.
  - ❌ Avoid cookies without HttpOnly+Secure flags.
...
|-------------------|---------------------|--------------------------------------------|--------------------------|
| **Access Token**  | 5–15 min            | Memory (browser) / Secure storage (mobile) | Call protected APIs      |
| **Refresh Token** | 7–30 days (rotated) | HttpOnly Secure Cookie / Secure OS storage | Obtain new access tokens |
